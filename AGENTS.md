...
- O projeto possui o modulo Ramais em `/ramais/` (apenas TI/admin): lista telefonica interna (model `Ramal`) com colaborador, setor, telefone, ramal e e-mail, busca dinamica e cadastro novo cujo e-mail vem de um `select` das contas ja cadastradas (`ContaEmail`), preenchendo o colaborador automaticamente. A lista inicial e semeada pela migration `0013` a partir de um arquivo local (`seed/ramais_seed.json`) ignorado pelo Git — os dados pessoais (nomes/telefones/e-mails) nao sao versionados.
