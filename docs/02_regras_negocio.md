...
5. O vinculo computador -> colaborador e resolvido **pelo nome do usuario do GLPI**, comparando o **conjunto de palavras** sem acento, em quatro passos: (a) conjunto identico; (b) um nome contido no outro com pelo menos 2 palavras em comum - resolve a inversao ("Gabriele Ana" x "Ana Gabriele") e nomes do meio a mais ("Garbuio Tamara Cristiane" x "Tamara Garbuio"); (c) 1 palavra igual + 1 palavra **parecida**, para grafia diferente do sobrenome ("Vich Everaldo" x "Everaldo Vichi", "Giamlourenco Marcelo" x "Marcelo Giannourenco"), com semelhanca minima de 82% e desprezando palavras de menos de 4 letras; (d) nome de **uma palavra so** (ex.: "portaria") casa apenas por igualdade exata; (e) por ultimo, a **ponte pelo nome completo da conta de e-mail corporativo**: o ramal pode estar cadastrado com nome curto ("Joao Leal") e o GLPI trazer outra combinacao ("Leal Henrique"), mas o nome completo do Workspace ("Joao Henrique Gomes Leal") contem os dois - achando a conta unica, o ramal e localizado pelo **e-mail**, que e chave exata. Em todos os passos exige-se **um unico** candidato. Na duvida, o computador fica sem dono e o vinculo e feito a mao clicando no computador na tela - **vinculo errado e pior que vinculo nenhum**, porque a pessoa sai silenciosamente do relatorio de quem esta sem antivirus (foi o caso de "Silva Andre", com varias "Silva" na lista).
...
7a. O **colaborador e o setor sao puxados automaticamente do inventario do GLPI** (modulo Contatos), casando pelo **nome da maquina** — a nomenclatura e a mesma nos dois sistemas (CPU-010, NOT-362, ...). Isso acontece nas duas importacoes (a do Kaspersky e a do GLPI), em qualquer ordem, e tambem pelo botao **"Puxar nomes do GLPI"** no topo da tela, para quando os dois arquivos ja foram importados antes. So preenche o que esta **em branco** e nunca troca um vinculo feito a mao.
8. A aba **"Colaboradores"** usa a **mesma populacao de pessoas do modulo Contatos**: os **Ramais** mais quem existe **so no inventario do GLPI** (marcado com a etiqueta "sem ramal cadastrado", com o setor vindo do local da maquina no GLPI). Sem isso, quem tem dispositivo no Kaspersky mas nao tem ramal nao aparecia na aba, e o numero de "sem colaborador" ficava sem explicacao na tela. Ela cruza essas pessoas com os dispositivos **e com o inventario do GLPI** (modulo Contatos) e mostra a situacao de cada pessoa: **Com antivirus** (tem dispositivo no Kaspersky com o antivirus instalado), **Sem antivirus** (tem dispositivo no Kaspersky sem o antivirus **ou** tem computador no GLPI que o Kaspersky nao conhece) e **Sem dispositivo** (nao tem maquina em nenhum dos dois). O computador que existe no GLPI mas nao tem registro no Kaspersky aparece marcado com **"(sem registro no Kaspersky)"**. A etiqueta evita confusao com o "so no GLPI" do modulo Contatos, que marca outra coisa: a **pessoa** que nao tem ramal cadastrado.
8a. Essa juncao com o GLPI corrige uma leitura enganosa: antes, quem tinha computador no inventario mas nenhum registro no Kaspersky caia em **"Sem dispositivo"**, escondendo **maquina sem antivirus** no meio de quem realmente nao tem computador. Com os dados de 29/07/2026, os "49 sem dispositivo" viraram **24 sem antivirus** (tem computador no GLPI, nada no Kaspersky) e **25 sem maquina** de fato.
9. As duas abas tem **apenas o campo de pesquisa** - nao ha mais chips de filtro por situacao nem por setor. A busca cobre tudo o que interessa e filtra a cada tecla: nome do dispositivo, setor, responsavel, colaborador vinculado, IP, status, versoes, grupo, observacoes, ultima conexao e tambem as **palavras das situacoes** - digitar "critico", "sem antivirus", "sem conexao", "fora do export" (ou, na aba Colaboradores, "com antivirus", "sem dispositivo") filtra a lista como os chips faziam. Aceita varias palavras (todas precisam bater), ignora acentos e maiusculas, e `Esc` limpa. A ordenacao por cabecalho continua na aba Dispositivos. "Sem conexao" e o dispositivo sem contato ha 7 dias ou mais.
