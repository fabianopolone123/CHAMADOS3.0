...
10a. **Excecao ao Play obrigatorio: chamado parado em "aguardando" (usuario/peca/autorizacao) pode ser encerrado direto pelo Stop, sem Play.** O que estava sendo esperado muitas vezes so se resolve sozinho (a peca nao veio mais, o usuario desistiu, a autorizacao saiu), e obrigar um Play so para fechar criaria um periodo de atendimento artificial. Por isso o card em "aguardando" sem Play mostra **Play e Stop** (o Pause fica oculto, pois nao ha atendimento rodando). Continua valendo tudo o mais: so TI/Admin (`403`), "O que foi feito" obrigatorio, chamado nao encerrado (`409` se ja estiver) e, fora de "aguardando" sem Play, o Stop e recusado com `409` ("Inicie o atendimento (Play) antes de finalizar este chamado."). O "Pause" sem Play continua bloqueado.
