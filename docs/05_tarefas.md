...
- Criado o modulo "Ramais" (apenas TI/admin): lista telefonica interna (colaborador, setor, telefone, ramal, e-mail) com busca dinamica, tabela responsiva (cards no celular) e cadastro novo cujo e-mail e um select das contas ja cadastradas (`ContaEmail`), com autopreenchimento do colaborador. Model `Ramal` (migration `0012`) + seed inicial via migration `0013` a partir de arquivo local ignorado pelo Git (`seed/ramais_seed.json`, dados pessoais nao versionados); rotas `/ramais/`, `/ramais/criar/`; testes `RamalCreateTests`
