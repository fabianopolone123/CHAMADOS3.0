...
- A lista (`/contratos/`) tem uma **pesquisa inteligente client-side**: a view monta, para cada requisicao, um texto pesquisavel (`_requisicao_busca_texto`) com codigo, titulo, tipo, status, descricao, autor, data, quem entregou e todos os orcamentos/suborcamentos (titulo, loja, link, valores) mais os nomes dos documentos anexados; esse texto vai no `data-search` do item e o filtro roda no navegador a cada tecla (sem rota nova e sem requisicao ao servidor), ignorando acentos/maiusculas e aceitando varias palavras. Como o texto vem da mesma view protegida por `ti_required`, o recorte de permissao continua no backend.
