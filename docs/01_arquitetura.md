...
- `core/api.py`: API para sistemas de fora — autenticacao por token (`TokenApiMiddleware`, que faz **qualquer rota** aceitar token no lugar da sessao) e os endpoints de leitura `/api/v1/tabelas/...` montados sobre o catalogo do painel. Escrita nao tem endpoint proprio: usa as rotas do modulo. Ver `docs/09_api.md`
- `core/xhr.py`: ponte entre as telas classicas e o terminal — o decorador `json_quando_xhr` faz a view de tela (que responde `redirect` + `messages`) devolver JSON quando o pedido vem por `fetch`, lendo as proprias mensagens dela; evita rota paralela so para o painel
- `core/painel_acoes.py`: catalogo das **acoes de fluxo** do painel — cada acao aponta para a mesma rota que a tela classica usa, para o terminal nao repetir regra de negocio; alem do envio comum, declara acao de **arquivo** (seletor do computador) e de **abrir** (aba nova do navegador)
