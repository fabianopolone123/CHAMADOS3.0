...
- Tabela do painel que existe para **trabalhar uma fila** (nao para consultar historico) usa `filtro` na `TabelaPainel` e mostra so o que esta pendente. Qualquer contagem dessa tabela tem de sair de `painel_dados.consulta_base()`, senao o numero do modulo diverge do que a lista mostra.
